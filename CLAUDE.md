...
6. No em dashes in user-facing text. Never put an em dash (the `—` character) in anything a visitor reads: page copy, headings, meta `description`/`content`, `alt`/`title` text, or button and link labels. Use a colon, period, comma, or parentheses instead. Em dashes inside code comments (HTML, CSS, JS) are fine because they never render.
